...
if(keyboard_check_released(ord("F")) && !instance_exists(obj_dialogController)){
	...
	if(global.countEvelyn == 0){
		...
		add_dialog("Good Morning Evelyn", true, c_blue, true)
		add_dialog("Morning", true, c_white, false)
		add_dialog("Anything come in for this morning?", true, c_blue, true)
		...
		add_dialog("Doesn’t make much sense, a hotel operates all day and night. Considering that employees walk through the halls periodically then someone would have seen them. If they just wanted money, then wouldn’t it be easier to catch him on the street?", true, c_blue, true)
		add_dialog("I don’t care, go waste his time, not mine. (Looks back at newspaper and sips coffee)", true, c_white, false)
		add_dialog("(This was always like Evelyn. She seemingly hates this job but refuses to quit. I'd fire her if she didn’t actually do a decent job of keeping documents in check. She's so oblivious to what actually happens here. Blissful ignorance.)", true, c_blue, true)
		add_dialog("Well are you going to go check the hotel or keep staring at my desk?", true, c_white, false)
		add_dialog("Er… Right, I'll be on my way.", false, c_blue, true)
	start_dialog(self, 0)
	}
	...
	dialog = []
	dialog_line = 0
		add_dialog("Any calls for, me?", true, c_blue, false)
